...
module.exports = {
  ...
  create: function (context) {
    ...
    const whitelist = getOption(context, 'whitelist')?.map(x => new RegExp(`^(${x})$`));
    ...
    const groups = groupUtil.getGroups(defaultGroups, mergedConfig)?.map(x => new RegExp(`^(${x})$`));
    const classnamesFromFiles = getClassnamesFromCSS(cssFiles, cssFilesRefreshRate)?.map(x => new RegExp(`^(${x})$`));
    ...
    const parseForCustomClassNames = (classNames, node) => {
      classNames.forEach((className) => {
        ...
        const whitelist2Idx = groupUtil.getGroupIndexSimple(className, whitelist);
        if (whitelist2Idx >= 0) {
          return; // Lazier is faster... processing next className!
        }
